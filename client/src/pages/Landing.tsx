    ...
    <main className="marketing-shell landing-page">
      ...
      <section className="landing-hero" style={{ backgroundImage: `linear-gradient(90deg, rgba(8,10,20,.98) 0%, rgba(8,10,20,.86) 35%, rgba(8,10,20,.28) 73%, rgba(8,10,20,.72) 100%), url(${REFERENCE_ART})` }}>
        <div className="hero-copy">
          ...
          <div className="hero-actions"><a className="primary-button hero-cta" href="/game">Accéder au cockpit <ArrowRight size={18} /></a><a className="text-link" href="/how-to-play">Voir comment ça marche <ArrowRight size={15} /></a></div>
      ...
      <section className="landing-intro page-section">
        ...
        <div className="intro-copy"><p>Chaque manche démarre à 1,00x et grimpe jusqu'au crash. Le pilote automatique mise, suit la courbe, et encaisse avant la fin du vol.</p><a className="text-link" href="/how-to-play">Apprendre les commandes <ArrowRight size={15} /></a></div>
      </section>
      <section className="feature-grid page-section">
        <a className="feature-card" href="/how-to-play"><span className="feature-icon"><Zap size={20} /></span><span className="eyebrow">01 / LA BOUCLE</span><h3>Une manche.<br />Une décision.</h3><p>Rapide à apprendre, impossible à ignorer. Votre timing écrit l'histoire.</p><span className="feature-link">Comment jouer <ArrowRight size={14} /></span></a>
        <a className="feature-card feature-card-gold" href="/fairness"><span className="feature-icon"><ShieldCheck size={20} /></span><span className="eyebrow">02 / LA PROMESSE</span><h3>Faites confiance<br />au vol.</h3><p>Comprenez la graine, vérifiez le résultat et rendez chaque manche auditable.</p><span className="feature-link">Notre modèle d'équité <ArrowRight size={14} /></span></a>
        <a className="feature-card" href="/history"><span className="feature-icon"><BarChart3 size={20} /></span><span className="eyebrow">03 / LE REGISTRE</span><h3>Voyez chaque<br />atterrissage.</h3><p>Rejouez les chiffres, étudiez vos sorties et trouvez votre propre rythme.</p><span className="feature-link">Voir l'historique des vols <ArrowRight size={14} /></span></a>
      </section>
      <section className="landing-final page-section"><div><span className="eyebrow"><Sparkles size={13} /> PRÊT QUAND VOUS L'ÊTES</span><h2>Votre meilleure sortie<br /><em>est encore devant vous.</em></h2></div><a className="primary-button" href="/game">Lancer Chicken Crash <Plane size={17} /></a></section>
      <footer className="marketing-footer"><span>Chicken Crash <span className="muted-divider">•</span> Contrôle de vol pour réflexes affûtés.</span><span><a href="/fairness">Équité</a><span className="muted-divider">•</span><a href="/how-to-play">Comment jouer</a></span></footer>
    </main>
  );
}
